...
NOTE ON morlet2 DEPRECATION:
  scipy.signal.morlet2 was removed in SciPy >= 1.12.
  This module uses a hand-rolled complex Morlet wavelet instead,
  which is identical in behaviour and has no external dependencies.
"""
import numpy as np
...
# ---------------------------------------------------------------------------
# Morlet wavelet (replaces deprecated scipy.signal.morlet2)
# ---------------------------------------------------------------------------

def _morlet_wavelet(n_cycles: float, f: float, fs: int) -> np.ndarray:
    """Build a complex Morlet wavelet for frequency *f* (Hz).

    The wavelet is scaled so that its peak power == 1.
    Length is chosen to capture 99.9 % of the Gaussian envelope.

    Parameters
    ----------
    n_cycles : float
        Number of oscillation cycles within the Gaussian window.
    ...
    """
    sigma = n_cycles / (2.0 * np.pi * f)   # Gaussian width in seconds
    # Choose half-length so Gaussian decays to < 0.1 % at edges
    half_len = int(3.0 * sigma * fs)
    t = np.arange(-half_len, half_len + 1) / fs
    gaussian = np.exp(-0.5 * (t / sigma) ** 2)
    carrier  = np.exp(2j * np.pi * f * t)
    wavelet  = gaussian * carrier
    # Normalise so power (sum |w|^2) == 1
    wavelet /= np.sqrt(np.sum(np.abs(wavelet) ** 2))
    return wavelet


# ---------------------------------------------------------------------------
# Core TFR computation
# ---------------------------------------------------------------------------
...
def morlet_power(
    ...
    n_cycles : float, number of wavelet cycles
    ...
    """
    n = len(signal)
    power = np.zeros((len(freqs), n), dtype=np.float64)
    for i, f in enumerate(freqs):
        wav = _morlet_wavelet(n_cycles, f, fs)
        if len(wav) > n:
            # Wavelet longer than signal: trim to signal length
            mid = len(wav) // 2
            half = n // 2
            wav = wav[max(0, mid - half): mid + half + 1]
        conv = fftconvolve(signal, wav[::-1].conj(), mode='same')
...
def compute_event_locked_tfr(
    ...
    Baseline normalization:
      ...
      This gives percent change from baseline.
    ...
    -------
    dict with keys:
        'avg_power' : np.ndarray (n_freqs, n_times), baseline-normalised (%)
    ...
    # Baseline indices (relative to window start)
    ...
    if not all_power:
        return {
            'avg_power': np.zeros((len(freqs), n_win)),
            'times': times,
            'freqs': freqs,
            'n_events': 0,
    ...
    # Baseline normalise: percent change from baseline
